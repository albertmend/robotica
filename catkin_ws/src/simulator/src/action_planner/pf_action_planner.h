...
int pf_start_clips_node_action_planner(){
 ...
 file = "/src/action_planner/ViRBot_Cubes_ROS/ROS_pf.dat";
...
*/
float pf_get_angle(float ang,float c,float d,float X,float Y){
        float x,y;
        x=c-X;
        y=d-Y;
        if((x == 0) && (y == 0)) return(0);
...
void pf_get_distance_theta(float x,float y,float angle,float x1,float y1,float *distance,float *theta){
 ...
 *theta=pf_get_angle(angle,x,y,x1,y1);
...
void pf_action_planner(float px, float py, float theta, movement *movements){
       ...
       if(init_flg==1){
       		...
       		// It starts the communication with the Clips node
       		pf_start_clips_node_action_planner();
       		init_flg=0;
